...
- Large documents now import PDF pages incrementally into Room, use bounded document-level OCR summaries while retaining full per-page OCR/FTS data, memoize page-wide UI analysis, use set-backed large selections and O(1) search membership, and chunk bulk SQL mutations below conservative bind-variable limits.
