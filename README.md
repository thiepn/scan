...
- Process/device-recreation hardening persists pending scanner intent (new/append/insert/retake/rapid/ID-back) with a compact fail-closed saveable codec, while Room-backed processing/workflows already resume after interruption. Missing or corrupt page images render an explicit `Preview unavailable` fallback instead of a silent blank surface.
