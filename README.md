...
- Non-destructive scan enhancement editor with Original, Auto, Clean, Color, Grayscale, B&W, Notes, Receipt, and Whiteboard presets plus brightness, contrast, highlights, shadows, black/white point, warmth, saturation, sharpness, background whitening, and shadow removal controls.
- Full Restoration Engine V2 adds custom-CV illumination-field estimation, bounded white balancing, multiplicative uneven-light correction, local contrast recovery, adaptive B&W, and document-profile tuning for Receipt, Whiteboard, Book, Notes, Form, and general documents.
- Restoration analysis runs on a tiny smoothed derivative while corrections are applied deterministically at full render resolution; canonical page images remain immutable, old v1 enhancement recipes remain readable, and new scan pages receive mode-aware v2 restoration defaults.
- Accessibility, Large-Document & Device Hardening adds single-target TalkBack selection semantics, navigable page/document headings, polite processing announcements, large-font library layouts, and page actions separated from titles so controls remain reachable under text scaling.
- Device capability budgets use Android memory class and low-RAM classification to bound library/page/restoration previews, imported-PDF derivatives, OCR/Book semantic intermediates, and edited-page export rasters. Untouched direct JPEG/native-PDF original-quality paths remain uncapped.
- Large documents now import PDF pages incrementally into Room, use bounded/streaming document-level OCR summaries while retaining full per-page OCR/FTS data, memoize page-wide UI analysis, use set-backed large selections and O(1) search membership, and chunk bulk SQL mutations below conservative bind-variable limits.
- Large exports avoid whole-document heap duplication: text, JSON, CSV, and XLSX output is streamed; generated and loaded PDFs use PDFBox temp-file-backed scratch storage; signature validation streams the signed ByteRange from disk instead of loading the complete PDF into a byte array.
- Low-storage hardening reserves 64 MiB of emergency free space and preflights conservative working sets before known-size PDF imports, page copies, PDF generation/editing/merge, signing, standards validation, and signature inspection. Failed staged copies delete temporary files, and initial scan construction rolls back partial Room/file state instead of leaving half-created documents.
- Process/device-recreation hardening persists pending scanner intent (new/append/insert/retake/rapid/ID-back) with a compact fail-closed saveable codec, while Room-backed processing/workflows already resume after interruption. Two-sided ID capture commits and recognizes the front as a real document before the back scanner opens, so cancellation/process recreation cannot evict or lose the front. Missing or corrupt page images render an explicit `Preview unavailable` fallback instead of a silent blank surface.
- Cold-start secure-vault sealing now runs off the main thread but remains fail-closed: protected documents begin locked/busy, seal failures surface as integrity warnings, unlock is disabled until sealing finishes, and persisted OCR/PDF/Book processing resumes asynchronously after a successful unlock.
- Room schema export is enabled through the Room 2.8 Gradle plugin with a committed schema directory target, so future schema snapshots can be versioned and used for migration validation.
- Inserted PDFs use the same conservative raster-workspace storage preflight as imported PDFs, preventing large insert jobs from starting when there is insufficient scratch space.
- Partial PDF imports preserve already committed pages and are flagged Needs Review instead of appearing silently complete; oversized PDF pages are downscaled to the active device budget and temporary bitmaps remain page-scoped.
...
- Smart Intake & Document Workflows add reusable processing presets, extraction/OCR/type-aware intake rules, automatic naming templates, folder/tag filing, classification/review/favorite/archive actions, compliance/security policy application, and manual batch execution.
- Scan-to-destination workflows use persisted Storage Access Framework folder grants for PDF, standardized PDF, privacy PDF, text, CSV, JSON, or XLSX delivery without broad storage permission.
- Workflow runs are durable Room records with ordered rule priority, one-shot intake idempotency, execution history, explicit manual retry, bounded exponential retry, interrupted-run recovery, and startup resumption.
